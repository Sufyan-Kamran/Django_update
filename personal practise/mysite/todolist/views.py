...
def preview(request,id):
    if request.user.is_authenticated:
        ...
        tfetch = donetask.objects.get(id=request.user.id)
...
def login(request):
    if request.method=="POST":
        ...
        if loginusername != None and loginpassword != None:
            ...
            if user is not None:
                ...
                return redirect('login')
        elif loginusername != None or loginpassword != None:
            error = True
            return render(request,"todolist/login.html", {"error":error})
        else:
            return render(request,"todolist/login.html")
    return render(request,"todolist/login.html")
def logout(request):
    ...
    if request.method=="POST":
        ...
        if loginusername != None and loginpassword != None:
            ...
            if user is not None:
                ...
                #return HttpResponse(request.user.staff)
            else:
                return redirect('login')
        elif loginusername != None or loginpassword != None:
            error = True
            return render(request,"todolist/manager.html", {"error":error})
        else:
            return render(request,"todolist/manager.html")
    return render(request,"todolist/manager.html")
def staff(request):
    ...
    for u in users:
        #print(u.username)
        a = notice.objects.filter(username=u.username).count()
        tnotices = notice.objects.filter(uid=u.id).count()
        upda = user.objects.get(id=u.id)        
        upda.notices = tnotices
        upda.save()
        print(u.id)
    ranges = [1, 2, 3, 4]
    params = {"notice" : notices,"users":users,"range": ranges, "cuser":tnotices,"nt":dic}
    return render(request,'todolist/staff.html',params )
def stud_view(request, myid):
    tnotices = notice.objects.filter(uid=2)
    userview = user.objects.filter(id=myid)
    donetsk = donetask.objects.filter(uid=myid)
    
    params = {"user":userview, "tnotice":tnotices,"donetask":donetsk}
    return render(request, "todolist/studview.html", params)